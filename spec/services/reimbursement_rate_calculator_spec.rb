...
RSpec.describe ReimbursementRateCalculator do
  ...
  describe "#reimbursements_due_for_trips" do
    it "returns a hash of reimbursements due for a set of trips" do
      reimbursements = @calculator.reimbursements_due_for_trips @run.trips
    ...
    it "calculates the cost of OAA trips" do
      ...
      expect(@calculator.reimbursements_due_for_trips(@run.trips)[:oaa3b]).to eq 1
    end
    
    it "calculates the cost of Ride Connection trips" do
      create :trip, run: @run, funding_source: @ride_connection
      expect(@calculator.reimbursements_due_for_trips(@run.trips)[:rc]).to eq 2
    end
    
    it "calculates the cost of TriMet trips" do
      create :trip, run: @run, funding_source: @trimet_non_medical
      expect(@calculator.reimbursements_due_for_trips(@run.trips)[:trimet]).to eq 4
    end
    
    it "calculates the cost of STF Van trips" do
      create :trip, run: @run, funding_source: @stf
      expect(@calculator.reimbursements_due_for_trips(@run.trips)[:stf_van]).to eq 8
    end
    
    it "calculates the cost of STF Taxi trips" do
      create :cab_trip, run: @run, funding_source: @stf
      expect(@calculator.reimbursements_due_for_trips(@run.trips)[:stf_taxi][:administrative]).to eq 16
    end
    ...
    it "calculates the cost of STF Taxi Wheelchair trips" do
      ...
      expect(@calculator.reimbursements_due_for_trips(@run.trips)[:stf_taxi][:wheelchair][:load_fee]).to eq 32
      expect(@calculator.reimbursements_due_for_trips(@run.trips)[:stf_taxi][:wheelchair][:mileage]).to eq 64
    end
    ...
    it "calculates the cost of STF Taxi Ambulatory trips" do
      ...
      expect(@calculator.reimbursements_due_for_trips(@run.trips)[:stf_taxi][:ambulatory][:load_fee]).to eq 128
      expect(@calculator.reimbursements_due_for_trips(@run.trips)[:stf_taxi][:ambulatory][:mileage]).to eq 256
    end
  end

  describe "#reimbursement_due_for_run" do
    it "should return 0 when the run has no trips" do
      expect(@calculator.reimbursement_due_for_run @run).to eq 0
    end
    ...
    it "should return a total based on all applicable trips" do
      ...
      expect(@calculator.reimbursement_due_for_run @run).to eq 1 + 2 + 4 + 8 + (16 * 3) + 32 + 64 + 128 + 256
    end
  end
end
