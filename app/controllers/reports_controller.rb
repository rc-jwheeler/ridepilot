...
class ReportsController < ApplicationController
  ...
  def vehicles_monthly
    ...
    @vehicles.each do |vehicle|
      ...
      month_trips = Trip.for_vehicle(vehicle.id).for_date_range(@start_date.in_time_zone, @end_date.in_time_zone).completed
  ...
  def service_summary
    ...
    counts_by_purpose = Trip.for_provider(current_provider_id).for_date_range(@start_date.in_time_zone, @end_date.in_time_zone)
    ...
    @turndowns = Trip.turned_down.for_date_range(@start_date.in_time_zone, @end_date.in_time_zone).for_provider(current_provider_id).count
    ...
    prior_customers_in_fiscal_year = trip_customers.for_date_range(fiscal_year_start_date(@start_date).in_time_zone, @start_date.in_time_zone).map {|x| x.customer_id}
    customers_this_period = trip_customers.for_date_range(@start_date.in_time_zone, @end_date.in_time_zone).map {|x| x.customer_id}
  ...
  def show_trips_for_verification
    ...
    unless @trips.present?
      @trips = Trip.for_provider(current_provider_id).for_date_range(@query.start_date.in_time_zone, @query.end_date.in_time_zone).
  ...
  def cab
    ...
    @trips = Trip.for_provider(current_provider_id).for_date_range(@query.start_date.in_time_zone, @query.end_date.in_time_zone).for_cab.completed.order(:pickup_time)
  end
  ...
  def daily_manifest
    ...
    trips = Trip.scheduled.for_provider(current_provider_id).for_date(@date.in_time_zone).includes(:pickup_address, :dropoff_address, :customer, :mobility, {run: :driver}).order(:pickup_time)
    if @query.driver_id == -2 # All
  ...
  def daily_trips
    ...
    @trips = Trip.for_provider(current_provider_id).for_date(@date.in_time_zone).includes(:pickup_address,:dropoff_address,:customer,:mobility,{:run => :driver}).order(:pickup_time)
    @trips_by_customer = @trips.group_by(&:customer)
  end
  ...
  def cctc_summary_report
    ...
    trip_queries = {
      in_range: {
        all: Trip.for_provider(@provider.id).for_date_range(@start_date.in_time_zone, @end_date.in_time_zone),
        stf: {}
      },
      ytd: {
        all: Trip.for_provider(@provider.id).for_date_range(Date.new(@start_date.year, 1, 1).in_time_zone, @end_date.in_time_zone)
      }
    }
  ...
  def prep_with_cab
    ...
    trips = Trip.scheduled.for_provider(current_provider_id).for_date(@date.in_time_zone).includes(:pickup_address,:dropoff_address,:customer,:mobility,{:run => :driver}).order(:pickup_time)
    @cab_trips = Trip.for_cab.scheduled.for_provider(current_provider_id).for_date(@date.in_time_zone).includes(:pickup_address,:dropoff_address,:customer,:mobility,{:run => :driver}).order(:pickup_time)
